...
import { githubAccessPlugin } from './plugin';
...
describe('plugin', () => {
  it('should create and read TODO items', async () => {
    const { server } = await startTestBackend({
      features: [githubAccessPlugin],
    });

    await request(server).get('/api/github-access/todos').expect(200, {
      items: [],
    });

    const createRes = await request(server)
      .post('/api/github-access/todos')
      .send({ title: 'My Todo' });
    ...
    await request(server)
      .get('/api/github-access/todos')
      .expect(200, {
        items: [createdTodoItem],
      });

    await request(server)
      .get(`/api/github-access/todos/${createdTodoItem.id}`)
